 ...
 *******************************************************************************/
package org.jboss.tools.quarkus.core.utils;
...
public final class JobHelpers {
	...
	public static void waitForJobsToComplete(IProgressMonitor monitor) throws InterruptedException, CoreException {
		...
		try {
			...
			if (processed) {
				QuarkusCorePlugin.logInfo("Could not flush background processing queues: " + getProcessingQueues(jobManager));
			}
		} finally {
			jobManager.resume();
		}
	...
	public static void waitForJobs(IJobMatcher matcher, int maxWaitMillis) {
		...
		while(true) {
			...
			if (timeout) {
				QuarkusCorePlugin.logInfo("Timeout while waiting for completion of job: " + job);
				break;
			}
			job.wakeUp();
			try {
